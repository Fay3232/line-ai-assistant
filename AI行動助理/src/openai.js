...
export async function answerUserMessage({ text, location }) {
  ...
  try {
    return await runResponsesToolLoop(input, { location });
  } catch (error) {
    console.error(error);
...
function heuristicReply({ text, location }) {
  ...
  if (/天氣|下雨|氣溫|颱風/.test(message)) {
    return "天氣查詢功能已準備好，但還需要設定 OPENAI_API_KEY 與 CWA_API_KEY，之後就能查台灣各縣市天氣。";
  }

  if (/美食|餐廳|吃什麼|咖啡|拉麵|牛肉麵/.test(message)) {
    return location
      ? "美食查詢功能已準備好，但還需要設定 OPENAI_API_KEY 與 GOOGLE_PLACES_API_KEY，之後可依你分享的位置推薦附近餐廳。"
      : "可以，請先分享 LINE 位置或輸入城市/區域，例如「台北車站拉麵」。";
  }

  if (/\b[A-Z]{1,5}\b|[0-9]{4}|股票|股價|台股|美股/.test(message.toUpperCase())) {
    return "股票查詢功能已準備好，但還需要設定 OPENAI_API_KEY；美股另需 FINNHUB_API_KEY。資訊只供查詢，不構成投資建議。";
  }
